...
#include <WebServer.h>      // <-- เพิ่มเว็บเซิร์ฟเวอร์
...
#define FORCE_RANDOM_DATA     1   // 1 = ส่งค่าสุ่ม 0-100 ทุกค่า, 0 = อ่านจาก Modbus
...
// LED & API
#ifndef LED_PIN
  #define LED_PIN 2               // เปลี่ยนตามบอร์ด (ESP32 DevKit ทั่วไป = GPIO2)
#endif
#define API_TOKEN "changeme"      // <<< ตั้งให้ตรงกับหน้าเว็บ

// เกณฑ์กรองค่า (นอกช่วง -> NaN) — ใช้เมื่อ FORCE_RANDOM_DATA=0
#define MOI_MIN  0.0f
...
// ----- WDT safe wrapper -----
static volatile bool g_wdtReady = false;
static inline void WDT_RESET_SAFE() {
...
static void handleLed() {
  ...
  else if (state == "toggle") digitalWrite(LED_PIN, !digitalRead(LED_PIN));
  else { server.send(400, "application/json", "{\"ok\":false,\"msg\":\"state must be on|off|toggle\"}"); return; }

  server.send(200, "application/json",
              String("{\"ok\":true,\"led\":") + String(digitalRead(LED_PIN)) + "}");
}

static void handleRestart() {
  addCORS();
  String token = server.hasArg("token") ? server.arg("token") : "";
  if (token != API_TOKEN) { server.send(403, "application/json", "{\"ok\":false,\"msg\":\"invalid token\"}"); return; }
...
// ---------------- TaskLoRa: อ่าน Modbus หรือสุ่ม + ส่ง LoRa(Text) ----------------
void TaskLoRa(void *pv) {
  ...
  for (;;) {
    if (millis() >= next_ms) {
      ...
      // เริ่มจาก NaN ทั้งหมด
      float v_moi = NAN, v_temp = NAN, v_ec = NAN, v_pH = NAN, v_N = NAN, v_P = NAN, v_K = NAN;

#if FORCE_RANDOM_DATA
      // ========== โหมดสุ่ม 0-100 ทุกค่า ==========
      auto r01 = [](){ return (float)random(0, 1001) / 10.0f; };  // 0.0..100.0 step 0.1
      v_N    = r01();
      v_P    = r01();
      v_K    = r01();
      v_pH   = r01();   // ตามคำขอให้ 0-100 เช่นกัน
      v_temp = r01();
      v_moi  = r01();
      v_ec   = r01();
#else
...
void setup() {
  Serial.begin(115200);
  delay(200);
  Serial.println("\n=== ESP32 Sender + OTA + NodeID + REST ===");

  // ให้สุ่มไม่ซ้ำ
  randomSeed(esp_random());

  // ----- WDT: init ตั้งแต่ต้น (กัน 'task not found') -----
  esp_task_wdt_deinit();
  ...
  g_wdtReady = true;
  Serial.println("🐶 WDT enabled (loopTask).");
  ...
  digitalWrite(LED_PIN, LOW);  // เริ่มต้นปิด
  ...
  server.onNotFound([](){ addCORS(); server.send(404, "application/json", "{\"ok\":false,\"msg\":\"not found\"}"); });

  // เริ่มเว็บเซิร์ฟเวอร์ (ก่อน OTA เพื่อให้สั่งงานได้ทันที)
  server.begin();
  Serial.println("🌐 REST server started on http://" + WiFi.localIP().toString());

  // เช็ค OTA ครั้งแรกหลัง WiFi
  checkForOTAUpdate();

  Serial.println("🐶 WDT will also watch TaskLoRa.");
